...
export function useAppExtensions(): UseAppExtensionsReturn {
	...
	const fetchExtensions = useCallback(async (appId: string, forceRefresh = false) => {
		// Check cache first
		...
		try {
			...
			const extensions = await invoke<any[]>("get_installable_extensions", {
				appId,
			});

			// Map snake_case to camelCase and mark as not installed by default
			const extensionsWithStatus: AvailableExtension[] = extensions.map(ext => ({
